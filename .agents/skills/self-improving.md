...
2. Write important findings to `memory.md`:
   ```bash
   python3 scripts/context_hub.py memory add "[TAG] Finding/decision summary"
   ```
...
```bash
python3 scripts/context_hub.py memory add "[BUG] Failure description — Root cause — Fix/workaround"
```
...
2. **Record to memory.md**:
   ```bash
   python3 scripts/context_hub.py memory add "[DECISION] User instruction: {specific instruction} — Reason: {if known}"
   ```
