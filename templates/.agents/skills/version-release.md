...
# Verify JSON syntax
python -c "import json; json.load(open('VERSION.json'))" && echo "✅ Valid JSON"
...
```bash
python scripts/context_hub.py memory add "[DECISION] Released v${NEW_VERSION} — Branch: ${RELEASE_BRANCH}. Tests: pass. Baselines: verified. Tag: v${NEW_VERSION}"
```
