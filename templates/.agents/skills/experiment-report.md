...
```bash
python scripts/context_hub.py memory add "[TAG] ${EXPERIMENT_NAME} — ${one_line_summary}
...
```bash
# Performance-related
python scripts/context_hub.py annotate Performance_Baselines "[EXPERIMENT] ${summary}"

# Limitation / edge case
python scripts/context_hub.py annotate Known_Limitations "[INSIGHT] ${summary}"
```
