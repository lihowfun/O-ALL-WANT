...
python -c "import ${KEY_PACKAGE_1}; import ${KEY_PACKAGE_2}"
...
```bash
python scripts/context_hub.py memory add "[WORKAROUND] ${ERROR_MSG} — Cause: ${ROOT_CAUSE}. Fix: ${SOLUTION}"
```

### If new limitation discovered

```bash
python scripts/context_hub.py annotate Known_Limitations "[BUG] ${ERROR_MSG} — No fix yet, needs investigation. Symptoms: ${SYMPTOMS}"
```

### If known issue re-encountered

```bash
python scripts/context_hub.py memory add "[BUG] Re-encountered Known Limitation: ${BRIEF_DESC}. Suggest prioritizing fix (hit ${N} times now)"
```
